
const String langRussian = 'Русский';

/// Russian-Russia
final Map<String, String> intlRuRu = {

  // 'OK': 'OK',

  // 'Customer Service': 'Customer Service',
  // '@total friends': '@total friends',

  //
  //  Register
  //
  'Register': 'Регистрация аккаунта',
  'Import': 'Импорт аккаунта',
  'Name': 'Имя',
  'Nickname': 'Никнейм',
  'Your nickname': 'Ваш никнейм',

  'Please choose your avatar': 'Пожалуйста, выберите изображение для вашего аватара.',
  'Please input your nickname': 'Пожалуйста, введите свой никнейм.',
  'Please agree the privacy policy': 'Пожалуйста, ознакомьтесь и согласитесь с политикой конфиденциальности.',
  'Failed to import account': 'Не удалось импортировать аккаунт, пожалуйста, проверьте свои мнемокоды.',
  'Failed to generate ID': 'Не удалось сгенерировать ID.',

  'Mnemonic Codes': 'Мнемокоды',
  'MnemonicCodes::Description': 'Мнемокод - это приватный ключ для существующего аккаунта,'
      ' если у вас его нет, пожалуйста, нажмите кнопку "Регистрация" в правом верхнем углу, чтобы сгенерировать новый.',

  'Show': 'Показать',
  'Hide': 'Скрыть',
  'Accept': 'Принять',

  //
  //  Photo
  //
  'Camera': 'Камера',
  'Album': 'Альбом',
  'Gallery': 'Галерея',
  'Pick Image': 'Выбрать изображение',
  'Image File Error': 'Ошибка загрузки изображения',
  'Upload Failed': 'Ошибка загрузки',

  'Save to Album': 'Сохранить в альбом',
  'Sure to save this image?': 'Вы уверены, что хотите сохранить это изображение?',
  'Image saved to album': 'Изображение успешно сохранено в альбоме.',
  'Failed to save image to album': 'Не удалось сохранить это изображение в вашем альбоме.',

  //
  //  Time
  //
  'AM': 'AM',
  'PM': 'PM',
  'Yesterday': 'Вчера',

  'Monday': 'Понедельник',
  'Tuesday': 'Вторник',
  'Wednesday': 'Среда',
  'Thursday': 'Четверг',
  'Friday': 'Пятница',
  'Saturday': 'Суббота',
  'Sunday': 'Воскресенье',

  '@several seconds': '@several секунд',
  '@several minutes': '@several минут',
  '@several hours': '@several часов',
  '@several days': '@several дней',
  '@several months': '@several месяцев',

  'Daily': '24 часа',    // 'Ежедневно',
  'Anon': '3 дней',
  'Weakly': '7 дней',    // 'Еженедельно',
  'Monthly': '30 дней',  // 'Ежемесячно',
  'Manually': 'Вручную',

  'Burn After Reading': 'Сжечь после чтения',

  //
  //  Connection
  //
  'Waiting': 'Ожидание',
  'Connecting': 'Подключение',
  'Connected': 'Подключено',
  'Handshaking': 'Аутентификация',
  'Disconnected': 'Отключено',

  //
  //  Alerts
  //
  'Cancel': 'Отмена',
  'Confirm': 'Подтвердить',
  'Confirm Add': 'Подтвердить Добавление',
  'Confirm Delete': 'Подтвердить Удаление',
  'Confirm Share': 'Подтвердить Общий доступ',
  'Confirm Forward': 'Подтвердить Пересылку',

  'Continue': 'Продолжить',
  'Deny': 'Отказать',
  'Allow': 'Разрешить',

  'Success': 'Успех',
  'Error': 'Ошибка',

  'Fatal Error': 'Фатальная ошибка',

  'Blocked': 'Заблокировано',
  'Unblocked': 'Разблокировано',
  'Muted': 'Без звука',
  'Unmuted': 'Со звуком',
  'Permission Denied': 'Доступ запрещен',

  'Refresh Stations': 'Обновить Ретрансляционные станции',
  'Refreshing all stations': 'Самая быстрая станция будет автоматически подключена в следующий раз.',

  'Shared': 'Успешно общий доступ',
  'Forwarded': 'Успешно переслано',

  'Notice': 'Уведомление',
  'Input Name': 'Введите имя',
  'Input text message': 'Введите текст сообщения',

  // Tabs
  'Chats': 'Чаты',
  'Contacts': 'Контакты',
  'Me': 'Я',
  'Services': 'Услуги',
  'Service Bots': 'Сервисные боты',

  // Contacts
  'New Friends': 'Новые друзья',
  'Group Chats': 'Групповые чаты',
  'Blocked List': 'Список заблокированных',
  'Muted List': 'Список без звука',

  'Search User': 'Поиск пользователя',

  // Settings
  'Settings': 'Настройки',
  'Export': 'Экспорт аккаунта',
  'Mnemonic': 'Мнемокод',
  'Language': 'Язык',
  'Brightness': 'Яркость',
  'Network': 'Сеть',
  'Relay Stations': 'Ретрансляционные станции',
  'Open Source': 'Открытый исходный код',
  'Terms': 'Условия',
  'Privacy Policy': 'Политика конфиденциальности',
  'About': 'О нас',

  'Edit Profile': 'Редактировать профиль',
  'Change Avatar': 'Сменить аватар',
  'Update & Broadcast': 'Обновить и тра��слировать',

  'System': 'Система',
  'Light': 'Светлая',
  'Dark': 'Темная',

  //
  //  Chat Box
  //
  'Hold to Talk': 'Удерживайте, чтобы говорить',
  'Release to Send': 'Отпустите, чтобы отправить',
  'Release to Cancel': 'Отпустите, чтобы отменить',

  'View More Members': 'Просмотреть больше участников',
  'Group Members (@count)': 'Участники группы (@count)',
  'Non-Member': 'Не участник',
  'Image Not Found': 'Изображение не найдено',
  'Failed to load image @filename': 'Не удалось загрузить изображение "@filename".',

  'Forward Text': 'Переслать текстовое сообщение',
  'Forward Image': 'Переслать изображение',
  'Forward Video': 'Переслать видео',
  'Forward Web Page': 'Переслать веб-страницу',
  'Forward Name Card': 'Переслать визитку',

  'Text message forwarded to @chat': 'Текстовое сообщение переслано в "@chat".',
  'Failed to share text with @chat': 'Не удалось поделиться текстовым сообщением с "@chat".',

  'Image message forwarded to @chat': 'Изображение переслано в "@chat".',
  'Failed to share image with @chat': 'Не удалось поделиться изображением с "@chat".',

  'Video message forwarded to @chat': 'Видео переслано в "@chat".',
  'Failed to share video with @chat': 'Не удалось поделиться видео с "@chat".',

  'Web Page @title forwarded to @chat': 'Веб-страница "@title" переслана в "@chat".',
  'Failed to share Web Page @title with @chat': 'Не удалось поделиться веб-страницей "@title" с "@chat".',

  'Name Card @name forwarded to @chat': 'Визитка "@name" переслана в "@chat".',
  'Failed to share Name Card @name with @chat': 'Не удалось поделиться визиткой "@name" с "@chat".',

  'Chat Details': 'Сведения о чате',
  'Group Chat Details (@count)': 'Сведения о групповом чате (@count)',
  'Group Name': 'Название группы',
  'Owner': 'Владелец',
  'Administrators': 'Администраторы',
  'Invitations': 'Приглашения',

  'Select Participants': 'Выберите участников',
  'Select a Chat': 'Выберите чат',

  'Recall Message': 'Отозвать сообщение',
  'Sure to recall this message?': 'Вы действительно хотите отозвать это сообщение?'
      ' (эта операция может не увенчаться успехом)',

  'Delete Message': 'Удалить сообщение',
  'Sure to delete this message?': 'Вы уверены, что хотите удалить это сообщение?'
      ' (это действие необратимо)',

  'Video error': 'Ошибка видео',
  'Download not supported': 'Загрузка пока не поддерживается',

  'Encrypting': 'Шифрование',
  'Decrypting': 'Дешифрование',

  'Waiting to upload': 'Данные зашифрованы, ожидание отправки',
  'Waiting to send': 'Ожидание отправки (нажмите, чтобы повторить)',
  'No response': 'Нет ответа (нажмите, чтобы отправить снова)',
  'Stranded': 'Застрял (нажмите, чтобы отправить снова)',
  'Encrypted and sent to relay station': 'Зашифровано и отправлено на ретрансляционную станцию',
  'Message is rejected': 'Сообщение отклонено',
  'Safely delivered': 'Безопасно доставлено',
  'Safely delivered to @count members': 'Безопасно доставлено @count участникам',

  'Draft': 'Черновик',
  'Mentioned': 'Вас упомянули',

  'Translate': 'Перевести',

  //
  //  Video Player
  //
  'Video Player': 'Видео плеер',

  'Loading "@url"': 'Загрузка "@url" ...',
  'Failed to load "@url".': 'Не удалось загрузить "@url".',

  'Select TV': 'Выбрать ТВ',
  'TV not found': 'ТВ не найден',
  'Search again': 'Искать снова',
  'Refresh': 'Обновить',

  //
  //  Web Browser
  //
  'Cannot launch "@url".': 'Не удается запустить "@url".',
  'Failed to launch "@url".': 'Не удалось запустить "@url".',

  //
  //  Check Update
  //
  'Please update app (@version, build @build).': 'Пожалуйста, обновите приложение до последней версии (@version, сборка @build).',
  'Upgrade': 'Обновить',
  'Download': 'Скачать',

  //
  //  Profile
  //
  'Remark': 'Заметка',
  'Block Messages': 'Заблокировать сообщения',
  'Mute Notifications': 'Отключить уведомления',

  'Send Message': 'Отправить сообщение',
  'Clear History': 'Очистить историю',
  'Add Contact': 'Добавить контакт',
  'Share Contact': 'Поделиться контактом',
  'Delete Contact': 'Удалить контакт',
  'Quit Group': 'Покинуть группу',
  'Report': 'Пожаловаться',

  'Contact @name shared to @chat': 'Контакт "@name" поделен с "@chat".',
  'Failed to share contact @name with @chat': 'Не удалось поделиться контактом "@name" с "@chat".',

  'Profile is updated': 'Ваш профиль обновлен и транслирован всем друзьям!',
  'Failed to update profile': 'Не удалось обновить профиль.',

  'Failed to get private key': 'Не удалось получить закрытый ключ.',
  'Failed to get visa': 'Не удалось получить визу.',
  'Failed to save visa': 'Не удалось сохранить визу.',

  //
  //  Prompts
  //
  'Please input group name': 'Пожалуйста, введите название группы.',
  'Please input alias': 'Пожалуйста, введите псевдоним.',
  'Please review invitations': 'Пожалуйста, просмотрите приглашения сначала.',

  'Current user not found': 'Текущий пользователь не найден.',
  'Failed to add contact': 'Не удалось добавить контакт.',
  'Failed to remove contact': 'Не удалось удалить контакт.',
  'Failed to remove friend': 'Не удалось удалить друга.',

  'Failed to add administrators': 'Не удалось добавить администраторов.',

  'Invited by': 'Пригласил',
  'Invitation sent': 'Новое приглашение отправлено всем администраторам, теперь оно ожидает проверки.',

  'Sure to reject all invitations?': 'Вы уверены, что хотите отклонить все приглашения?',

  'Sure to add this friend?': 'Вы уверены, что хотите добавить этого друга?',
  'Sure to remove this friend?': 'Вы уверены, что хотите удалить этого друга?'
      ' Это действие также удалит историю чата.',
  'Sure to remove this group?': 'Вы уверены, что хотите удалить эту группу?'
      ' Это действие также удалит историю чата.',

  'Sure to clear chat history of this friend?': 'Вы уверены, что хотите очистить историю чата этого друга?'
      ' Это действие не может быть отменено.',
  'Sure to clear chat history of this group?': 'Вы уверены, что хотите очистить историю чата этой группы?'
      ' Это действие не может быть отменено.',
  'Failed to clear chat history': 'Не удалось очистить историю чата.',

  'Sure to remove this conversation?': 'Вы уверены, что хотите удалить этот разговор? Это действие не может быть отменено.',
  'Failed to remove conversation': 'Не удалось удалить разговор.',

  'Failed to set remark': 'Не удалось установить заметку.',

  'Never receive message from this contact': 'Вы больше не будете получать сообщения от этого контакта.',
  'Receive message from this contact': 'Теперь вы можете получать сообщения от этого контакта.',

  'Never receive notification from this contact': 'Вы больше не будете получать уведомления от этого контакта.',
  'Receive notification from this contact': 'Теперь вы можете получать уведомления от этого контакта.',

  'Grant to access external storage': 'Необходимо предоставить разрешение для доступа к внешнему хранилищу.'
      ' Пожалуйста, разрешите создание баз данных для локального хранилища.',
  'Grant to access photo album': 'Необходимо предоставить разрешение для доступа к фотоальбому.'
      ' Пожалуйста, разрешите отправлять фото из вашего альбома или менять фотографию профиля,'
      ' или сохранять изображение в ваш альбом.',
  'Grant to access camera': 'Необходимо предоставить разрешение для доступа к камере.'
      ' Пожалуйста, разрешите делать фотографии и отправлять их друзьям, или менять фотографию профиля.',
  'Grant to access microphone': 'Необходимо предоставить разрешение для доступа к микрофону.'
      ' Пожалуйста, разрешите записывать голосовое сообщение и отправлять его вашему другу.',
  'Grant to allow notifications': 'Необходимо предоставить разрешение для разрешения уведомлений.'
      ' Пожалуйста, разрешите уведомления для офлайн-сообщений.',

  'Notification': 'Уведомление',
  'Enabled': 'Включено',
  'Disabled': 'Отключено',

  'Failed to get image file': 'Не удалось получить файл изображения.',
  'Cannot save this image': 'Не удается сохранить это изображение.',

  'Failed to send command': 'Не удалось отправить команду.',

  'Sure to remove this station (@remote)?': 'Вы уверены, что хотите удалить эту станцию (@remote)?',
  'Station (@remote) is removed': 'Станция (@remote) удалена.',
  'Failed to remove station (@remote)': 'Не удалось удалить станцию (@remote).',
  'Cannot remove this station': 'Не удается удалить эту станцию.',

  'Invite command error': 'Ошибка команды приглашения.',
  'Expel command error': 'Ошибка команды изгнания.',

  'Unsupported group command: @cmd': 'Неподдерживаемая команда группы: @cmd.',

  '"@commander" reset group': '"@commander" обновил участников группы.',
  '"@commander" join group': '"@commander" хочет присоединиться к этой группе.',
  '"@commander" left group': '"@commander" покинул группу.',
  '"@commander" invite "@members"': '"@commander" приглашает "@members" в эту группу.',
  '"@commander" invite "@member"': '"@commander" приглашает "@member" в эту группу.',
  '"@commander" expel "@members"': '"@commander" исключает участников "@members" из группы.',
  '"@commander" expel "@member"': '"@commander" исключает участника "@member" из группы.',

  //
  //  Cache Files Management
  //

  'Storage': 'Хранилище',
  'Cache Files Management': 'Управление кэш-файлами',

  'Total Cached Data': 'Всего закэшированных данных',
  'Cache Files': 'Кэш-файлы',
  'Database': 'Базы данных',
  'Avatars': 'Аватары',
  'Message Files': 'Файлы сообщений',
  'Temporary Files': 'Временные файлы',
  'Upload Directory': 'Каталог загрузки',
  'Download Directory': 'Каталог загрузки',

  'Contains @count file, totaling @size': 'Содержит @count файл, общим размером @size.',
  'Contains @count files, totaling @size': 'Содержит @count файла, общим размером @size.',

  'Scan': 'Сканировать',
  'Clear': 'Очистить',

  'Sure to clear all avatar images?': 'Вы уверены, что хотите удалить все изображения аватаров?',
  'Sure to clear all message files?': 'Вы уверены, что хотите удалить все файлы сообщений? Эту операцию нельзя будет отменить!',
  'Sure to clear these temporary files?': 'Вы уверены, что хотите удалить эти временные файлы?',

  'CacheFiles::Description': '* Изображения аватаров и файлы сообщений хранятся на вашем устройстве.\n'
      '* Файлы сообщений, переданные по сети, были зашифрованы'
      ' и не могут быть расшифрованы промежуточными узлами, такими как ретрансляторы.\n'
      '* После удаления этих файлов из локального хранилища'
      ' вы, возможно, не сможете восстановить их, если не попросите отправителя повторно отправить их.\n'
      '* Обратите внимание, что наш файловый ретрансляторный сервер может кэшировать только зашифрованные версии'
      ' файлов сообщений на период до 7 дней.\n'
      '* Поэтому, пожалуйста, берегите эти данные.',

  'TemporaryFiles::Description': '* При загрузке или загрузке файлов приложение создает временные файлы,'
      ' которые обычно автоматически удаляются системой при необходимости.\n'
      '* Если вы хотите удалить их немедленно, вы также можете выбрать вручную удалить их здесь.',

  //
  //  Footers
  //

  'ServiceBotList::Description': '* Здесь отображаются проверенные сервис-боты, некоторые из них от сторонних поставщиков;\n'
      '* Если вы обнаружите любое незаконное поведение, нажмите кнопку "Пожаловаться" в правом верхнем углу, чтобы получить доказательства и сообщить;\n'
      '* Любой может разрабатывать и предоставлять такие услуги общественности, если он следует протоколу DIMP;\n'
      '* Если вы хотите создать своего собственного сервис-бота, мы приглашаем вас прочитать документы DIMP,'
      ' скачать примеры проектов с GitHub или напрямую связаться с "Albert Moky".',

  'ChatBox::Description': 'Это приложение работает на технологии DIM, обеспечивающей конечное шифрование (End-to-End Encrypted).'
      ' Ваши сообщения будут зашифрованы перед отправкой и смогут быть расшифрованы только получателем.',

  'ChatBox::Remind': 'Пожалуйста, не отправляйте никакую незаконную информацию в этом чат-приложении, включая, но не ограничиваясь, непристойным, мошенническим или угрожающим содержанием.'
      ' Если вы найдете незаконную информацию, пожалуйста, нажмите кнопку в правом верхнем углу, чтобы сообщить об этом.'
      ' Поддерживайте хорошую коммуникационную среду и совместно соблюдайте законы и правила. Спасибо!',

  'ChatList::Description': '* Здесь отображаются истории чатов только с вашими друзьями;\n'
      '* Незнакомцы будут помещены в раздел "Контакты -> Новые друзья".',

  'Strangers::Description': '* Здесь отображаются незнакомцы, которые хотят подружиться с вами;\n'
      '* Вы можете добавить их в свои контакты или просто проигнорировать.',

  'GroupList::Description': '* Здесь показаны истории чатов всех ваших групп.',

  'BlockList::Description': '* Здесь отображаются спамеры, которых вы заблокировали;\n'
      '* Вы больше не будете получать сообщения из этого списка.',

  'MuteList::Description': '* Здесь показаны шумные друзья, которые вам не очень ценны;\n'
      '* Вы все еще можете общаться с ними, но не будете получать уведомлений из этого списка.',

  'Mnemonic::Description': '* Мнемоническая фраза - ваш закрытый ключ, любой, кто узнает эти слова, сможет получить доступ к вашей учетной записи;\n'
      '* Вы можете записать ее на бумаге и хранить в надежном месте,'
      ' но не рекомендуется делать скриншот и сохранять на компьютере.',

  'Administrators::Description': 'Правила:\n'
      '  1. Владелец или администраторы могут просматривать приглашения;\n'
      '  2. Владелец или администраторы могут добавлять/удалять участников напрямую;\n'
      '  3. Владелец может нанимать/увольнять администраторов, администраторы могут увольняться сами;\n'
      '  4. Владелец может изменить название группы;\n'
      '  5. Владелец не может покинуть группу;\n'
      '  6. Администратор не может покинуть группу до момента ухода.',

  'Invitations::Description': 'Правила:\n'
      '  1. Владелец или администраторы могут добавлять участников напрямую;\n'
      '  2. Другие участники могут создавать приглашения, которые будут рассматриваться администраторами;\n'
      '  3. Любой администратор может подтвердить приглашения и обновить список участников.',

  'BurnAfterReading::Description': 'Правила:\n'
      '  1. Установите период времени, и приложение автоматически удалит все сообщения и файлы до этого времени;\n'
      '  2. Удаленные сообщения и файлы не могут быть восстановлены, поэтому используйте эту функцию осторожно;\n'
      '  3. При установке ручного режима сообщения и файлы больше автоматически не удаляются.',

  'RelayStations::Description': 'Правила:\n'
      '  1. В качестве текущей реле-станции будет выбрана станция с самым быстрым ответом;\n'
      '  2. Если выбрано несколько станций, будет выбрана самая быстрая из выбранных;\n'
      '  3. Если станции не выбраны, будет выбрана автоматически из всех станций.',

};
